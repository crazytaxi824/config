...
-- ---
-- --- `:help jobstart-options` env
-- --- @field env fun(): string|nil
-- ---
-- --- goto bottom of the terminal. 在 on_stdout & on_stderr 中触发.
-- --- @field auto_scroll fun(): boolean|nil
-- ---
-- --- true: 在 console 中执行; false: 在 terminal 中执行.
-- --- @field console_output fun(): boolean|nil
-- ---
-- --- term:run() 时触发. before jobstart().
-- --- @field before_run fun(): MyTermCallback[]|nil
-- ---
-- --- term:run() 时触发. 在 jobstart() 之后马上执行, 和 on_exit 的区别是不用等到 jobdone.
-- --- @field after_run  fun(): MyTermCBWithJob[]|nil
-- ---
-- --- BufWinEnter. NOTE: 每次 term:// buffer 被 win 显示的时候都会触发, 同一个 buffer 被多个窗口显示时也会触发.
-- --- @field on_open    fun(): MyTermCallback[]|nil
-- ---
-- --- BufWinLeave. NOTE: BufWinLeave 只会在 buffer 离开最后一个 win 的时候触发.
-- --- @field on_close   fun(): MyTermCallback[]|nil
-- ---
-- --- jobstart() 中 callback 函数
-- --- @field on_stdout  fun(): MyTermOnOutput[]|nil
-- ---
-- --- jobstart() 中 callback 函数
-- --- @field on_stderr  fun(): MyTermOnOutput[]|nil
-- ---
-- --- jobstart() 中 callback 函数
...
-- ---
-- --- @field from fun(myterm: MyTerm, bufnr: integer, job_id: integer): MyTermPost
