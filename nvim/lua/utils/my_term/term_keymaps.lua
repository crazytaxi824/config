...
function M.set_buf_keymaps(term_opts, term_bufnr)
  ...
  local keys = {
    {'n', '<leader>tc', function() close_others(term_opts.id) end,   opt, 'my_term: close other my_terms windows'},
    {'n', '<leader>tw', function() wipeout_others(term_opts.id) end, opt, 'my_term: wipeout other my_terms'},
    {'n', 'q', '<cmd>q<CR>', opt, 'my_term: close current my_term window'},
    {'n', 'Q', function() wipeout_term(term_opts.id) end, opt, 'my_term: wipeout current my_term'},
  }
