...
M = {
  [local_linter_key] = {
    ...
    -- -- }}}
    golangci_lint = diagnostics.golangci_lint.with(proj_local_settings.keep_extend(local_linter_key, 'golangci_lint',
    ...
    -- -- }}}
    eslint = diagnostics.eslint.with(proj_local_settings.keep_extend(local_linter_key, 'eslint',
      eslint_opts, diagnostics_opts)),

    --- python: flake8, mypy
    flake8 = diagnostics.flake8.with(proj_local_settings.keep_extend(local_linter_key, 'flake8', diagnostics_opts)),
    mypy = diagnostics.mypy.with(proj_local_settings.keep_extend(local_linter_key, 'mypy', {
      extra_args = {"--follow-imports=silent", "--ignore-missing-imports"},
    }, diagnostics_opts)),

    --- protobuf: buf
    buf = diagnostics.buf.with(proj_local_settings.keep_extend(local_linter_key, 'buf', diagnostics_opts)),
  },

  [local_formatter_key] = {
    prettier = formatting.prettier.with(proj_local_settings.keep_extend(local_formatter_key, 'prettier',
      require("user.lsp.null_ls.tools.prettier")  -- NOTE: 加载单独设置 null_ls/tools/prettier.lua
    )),

    --- python: autopep8, black, YAPF
    autopep8 = formatting.autopep8.with(proj_local_settings.keep_extend(local_formatter_key, 'autopep8', {})),
    ...
    goimports = formatting.goimports.with(proj_local_settings.keep_extend(local_formatter_key, 'goimports', {})),
    ...
    --- sh shell: shfmt
    shfmt = formatting.shfmt.with(proj_local_settings.keep_extend(local_formatter_key, 'shfmt', {})),

    --- protobuf: buf
    buf = formatting.buf.with(proj_local_settings.keep_extend(local_formatter_key, 'buf', {})),
  },

  [local_code_actions_key] = {
    ...
    eslint = code_actions.eslint.with(proj_local_settings.keep_extend(local_code_actions_key, 'eslint', eslint_opts)),
  },
}
