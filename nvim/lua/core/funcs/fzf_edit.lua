...
function FZF_selected(fzf_file)
  ...
  local fp_qf_list = parse_fzf_items(fzf_file)
  if not fp_qf_list then
    return
  end
  ...
  vim.api.nvim_win_set_cursor(win_id, {fp_qf_list[1].lnum, fp_qf_list[1].col-1})  -- {1,0}-based index
