...
;;; 给 struct{} 添加 private field 属性和颜色, eg: type Foo struct { bar string } 中的 bar (第一个字母是 lower case).
...
;;; 申明 struct{} 时 private field 的颜色. p := P{a:1, b:2}
...
(import_declaration
  (import_spec_list
    (import_spec path: ((interpreted_string_literal
      (interpreted_string_literal_content) @string.special.url)))))
