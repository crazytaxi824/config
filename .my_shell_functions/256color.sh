...
function fgGrayscale() {
	for ((color = 232; color < 244; color++)); do
		printf "\e[38;5;%dm %d " $color $color
	done
	printf "\e[0m\n"

	for ((color = 244; color < 256; color++)); do
		printf "\e[38;5;%dm %d " $color $color
	done
	printf "\e[0m\n\n"
}
...
function fgCube() {
	for ((line = 0; line < 6; line++)); do
		for ((cube = 0; cube < 3; cube++)); do # 每行 3 个 cube.
...
function bgGrayscale() {
	for ((line = 0; line < 2; line++)); do
		for ((color = 232; color < 244; color++)); do # from 0 to 255 step 1
			if ((line == 0)); then
	...
	for ((line = 0; line < 2; line++)); do
		for ((color = 244; color < 256; color++)); do # from 0 to 255 step 1
			if ((line == 0)); then
...
function bgCube() {
	for ((line = 0; line < 12; line++)); do # 12 行, 双数行 color code, 单数行空格.
		for ((cube = 0; cube < 3; cube++)); do # 每行 3 个 cube.
			for ((cell = 0; cell < 6; cell++)); do
				# 以下两种写法都可以
